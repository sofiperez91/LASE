...
d = 2
MODEL_FILE = (
    "../saved_models/lase_unshared_d2_normalized_unbalanced_rand_10steps_M01.pt"
)
...
for step in range(gd_steps):
    ...
    model.gd[step].gcn.lins[1].weight.data = (
        torch.nn.init.xavier_uniform_(model.gd[step].gcn.lins[1].weight) * lr
    )

    # TransformerBlock
    model.gd[step].gat.lin2.weight.data = lr * torch.nn.init.xavier_uniform_(
        model.gd[step].gat.lin2.weight.data
    ).to(device)
...
for data in df_train_:
    ...
    for i in selected_nodes:
        ...
        M01[:, i] = votos
    df_train.append(
        Data(x=data.x, edge_index=data.edge_index, mask=M01.nonzero().t().contiguous())
    )

with open(VAL_DATA_FILE, "rb") as f:
    df_val_ = pickle.load(f)
...
for epoch in range(epochs):
    ...
    for i, batch in enumerate(train_loop):
        ...
        loss = torch.norm(
            (out @ out.T - to_dense_adj(batch.edge_index).squeeze(0))
            * to_dense_adj(batch.mask).squeeze(0)
        )
        loss.backward()
        optimizer.step()
